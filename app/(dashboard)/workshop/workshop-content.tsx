...
import { Blocks, CheckCircle, Eye, PartyPopper, Save, X, Sparkles } from "lucide-react";
    ...
    <AnimatePresence>
      {status === "saved" && (
        <motion.div
          ...
          className="rounded-lg bg-emerald-100 px-3 py-1.5 text-sm font-medium text-emerald-700"
        >
          Project saved!
        </motion.div>
      )}
      {status === "error" && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -8 }}
          transition={{ duration: 0.2 }}
          className="rounded-lg bg-destructive/10 px-3 py-1.5 text-sm font-medium text-destructive"
        >
...
export function WorkshopContent({ lesson, chatPanel }: WorkshopContentProps) {
  ...
  const [showStory, setShowStory] = useState(!!lesson?.storyText);
